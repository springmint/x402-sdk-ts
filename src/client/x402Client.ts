...
import type { PaymentRequirements, PaymentPayload, Permit402Context } from "../types/index.js";
...
export interface ClientMechanism {
  ...
  createPaymentPayload(
    ...
    resource: string,
    extensions?: { permit402Context?: Permit402Context },
...
export type PaymentRequirementsSelector = (requirements: PaymentRequirements[]) => PaymentRequirements;
...
export class X402Client {
  ...
  registerPolicy(policy: PaymentPolicy | { new (client: X402Client): PaymentPolicy }): X402Client {
    const instance =
      typeof policy === "function" ? new (policy as { new (client: X402Client): PaymentPolicy })(this) : policy;
  ...
  async handlePayment(
    ...
    const requirements = selector ? selector(accepts) : await this.selectPaymentRequirements(accepts);
